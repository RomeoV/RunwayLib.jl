using Test
using RunwayLib
import RunwayLib: camera_config_to_matrix, _ustrip, validate_camera_matrix
...
@testset "Camera Model" begin
    ...
    @testset "CameraMatrix Functionality" begin
        @testset "CameraMatrix Construction" begin
            ...
            # Use the camera_config_to_matrix function to create a proper matrix
            matrix = camera_config_to_matrix(CAMERA_CONFIG_OFFSET).matrix
        ...
        @testset "CameraMatrix Projection" begin
            # Create a camera matrix equivalent to CAMERA_CONFIG_OFFSET using the conversion function
            cam_matrix = camera_config_to_matrix(CAMERA_CONFIG_OFFSET)
            ...
            @test abs(ustrip(proj_config.x) - ustrip(proj_matrix.x)) < 1e-6
            @test abs(ustrip(proj_config.y) - ustrip(proj_matrix.y)) < 1e-6
            ...
            @test abs(ustrip(proj_config2.x) - ustrip(proj_matrix2.x)) < 1e-6
            @test abs(ustrip(proj_config2.y) - ustrip(proj_matrix2.y)) < 1e-6
        end
        
        @testset "camera_config_to_matrix Conversion" begin
            # Test conversion from CameraConfig to CameraMatrix
            matrix_config_offset = camera_config_to_matrix(CAMERA_CONFIG_OFFSET)
            matrix_config_centered = camera_config_to_matrix(CAMERA_CONFIG_CENTERED)
            ...
            @test abs(ustrip(proj_orig_offset.x) - ustrip(proj_conv_offset.x)) < 1e-6
            @test abs(ustrip(proj_orig_offset.y) - ustrip(proj_conv_offset.y)) < 1e-6
            ...
            @test abs(ustrip(proj_orig_centered.x) - ustrip(proj_conv_centered.x)) < 1e-6
            @test abs(ustrip(proj_orig_centered.y) - ustrip(proj_conv_centered.y)) < 1e-6
        end
        ...
        @testset "Pose Estimation with CameraMatrix" begin
            ...
            camera_matrix = camera_config_to_matrix(CAMERA_CONFIG_OFFSET)
