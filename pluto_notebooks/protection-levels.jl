...
# ╔═╡ abeca8c5-eea3-4d19-9f2f-10a91c03e542
begin
	using RunwayLib
...
# ╔═╡ 3c5dfa8b-11d7-429c-b7f6-d520c03d6713
md"""
# Protection Levels for Vision-Based Landing

This notebook demonstrates how to compute **protection levels** (PLs) for a runway pose estimation system. A protection level bounds the worst-case position error that could go undetected by the integrity monitor.

We use two solvers:
- **Ipopt** (gradient-based, fast, ~200ms)
- **NelderMead** (derivative-free, slower, ~8s)

Both solve the same bilevel optimization: find the worst-case measurement perturbation that maximizes position error while remaining undetected.
"""

# ╔═╡ 48826d10-0256-430a-952f-fa90ecf418b2
md"## Problem Setup"

# ╔═╡ ce444d71-4898-43dd-a289-4273c047d2cc
begin
	# Runway geometry: 4 corners of a 1500m × 50m runway
	runway_corners = [
	...
	# Measurement noise: sqrt(2) pixel std dev per coordinate
	px_std = sqrt(2.0)
	noise_cov = Diagonal(px_std^2 * ones(8))

	md"Runway: **1500m × 50m**, noise std: **$(round(px_std, digits=2)) px**"
end

# ╔═╡ 4776f35c-4e09-49e1-ba23-9e5f1d739980
begin
	# Aircraft state: position and attitude
	rng = MersenneTwister(42)
	cam_pos = WorldPoint(-2500.0m, 10.0m, 250.0m)
	cam_rot = RotZYX(0.02, 0.05, 0.01)

	md"""**Aircraft position**: $(cam_pos)

**Attitude** (yaw, pitch, roll): $(round.(rad2deg.(Rotations.params(cam_rot)), digits=1))°"""
end

# ╔═╡ f3c7f1d2-fabc-4f14-b506-3ea49a13dbaa
md"## Observations"

# ╔═╡ 1f62723c-84f1-4f26-99a5-e904bf7136fd
begin
	# Project runway corners into image, then add noise
	clean_obs = [project(cam_pos, cam_rot, c) for c in runway_corners]
	noisy_obs = clean_obs .+ [ProjectionPoint(px_std * randn(rng, 2)px) for _ in clean_obs]

	_r(x) = round(typeof(1.0px), x, digits=1)
	_obs_table = join(["| $i | ($(_r(c.x)), $(_r(c.y))) | ($(_r(n.x)), $(_r(n.y))) |"
		for (i, c, n) in zip(1:4, clean_obs, noisy_obs)], "\n")

	Markdown.parse("""Projected 4 runway corners into the image and added Gaussian noise.

| Corner | Clean (x, y) | Noisy (x, y) |
|--------|-------------|--------------|
$(_obs_table)
""")
end

# ╔═╡ a1791dcf-e615-4c62-9e13-140645670ae8
md"## Pose Estimation"

# ╔═╡ 11f4a6f5-2bd6-49db-895f-6f3c0e2afe3f
begin
	# 3-DOF pose estimation (position only, attitude known)
	pose = estimatepose3dof(PointFeatures(runway_corners, noisy_obs), NO_LINES, cam_rot)

	pos_err = pose.pos .- cam_pos
	md"""**Estimated position**: $(pose.pos)

**Position error**: $(pos_err)"""
end

# ╔═╡ d0e0d47c-e8a1-4136-add8-e01f957f46db
md"## Integrity Check"

# ╔═╡ e34a688c-7ee8-4334-b3fd-ae9c75da7d73
begin
	sr = compute_integrity_statistic(pose.pos, pose.rot, runway_corners, noisy_obs, noise_cov)

	md"""The integrity statistic tests whether measurements are consistent with the model.

| Metric | Value |
|--------|-------|
| Test statistic | $(round(sr.stat, digits=3)) |
| p-value | $(round(sr.p_value, digits=4)) |
| Degrees of freedom | $(sr.dofs) |

**Verdict**: $(sr.p_value > 0.05 ? "✓ Consistent (p > 0.05)" : "✗ Inconsistent (p ≤ 0.05)")
"""
end

# ╔═╡ 10e3161e-c68c-414c-8d7b-67a84814b065
md"## Protection Levels"

# ╔═╡ 1ba8f3ed-be56-48b4-9015-fadfafa8e7e1
md"""
Computing protection levels for all three position axes using the **Ipopt** solver.
The PL answers: *what is the largest position error that could go undetected?*
"""

# ╔═╡ b38ad890-f716-451b-bf9e-4ee66a8569f1
begin
	axis_labels = ["Along-track", "Cross-track", "Altitude"]
	prob = 0.01  # 1% false alarm rate
	...
	_ipopt_table = join(["| $(axis_labels[i]) | $(round(results_ipopt[i].lo.protection_level, digits=2)) | $(round(results_ipopt[i].hi.protection_level, digits=2)) | $(results_ipopt[i].lo.feasible && results_ipopt[i].hi.feasible ? "yes" : "no") |" for i in 1:3], "\n")

	Markdown.parse("""### Ipopt Results (gradient-based)

| Axis | PL- (m) | PL+ (m) | Feasible |
|------|---------|---------|----------|
$(_ipopt_table)
""")
end

# ╔═╡ e4c03661-6c87-4b18-b643-5365b29747af
md"Now comparing with the **NelderMead** solver (derivative-free, slower)."

# ╔═╡ eaa9aa29-80e9-438d-a1bc-fb058852da45
begin
	results_nm = [
	...
	_nm_table = join(["| $(axis_labels[i]) | $(round(results_nm[i].lo.protection_level, digits=2)) | $(round(results_nm[i].hi.protection_level, digits=2)) |" for i in 1:3], "\n")

	Markdown.parse("""### NelderMead Results
...
# ╔═╡ 8d13a836-5a4a-4274-bac5-67019ce50232
md"### Comparison Plot"

# ╔═╡ 9b298676-5ecf-425b-8c87-793147e97421
begin
	pl_ipopt_lo = [r.lo.protection_level for r in results_ipopt]
	pl_ipopt_hi = [r.hi.protection_level for r in results_ipopt]
	pl_nm_lo = [r.lo.protection_level for r in results_nm]
	pl_nm_hi = [r.hi.protection_level for r in results_nm]

	xs = 1:3
	w = 0.2
	p = plot(; title="Protection Levels: Ipopt vs NelderMead",
		ylabel="Protection Level (m)", xticks=(xs, axis_labels), legend=:topright)
	bar!(p, xs .- 1.5w, pl_ipopt_lo; width=w, label="Ipopt (lo)")
	bar!(p, xs .- 0.5w, pl_nm_lo; width=w, label="NM (lo)")
	bar!(p, xs .+ 0.5w, pl_ipopt_hi; width=w, label="Ipopt (hi)")
	bar!(p, xs .+ 1.5w, pl_nm_hi; width=w, label="NM (hi)")
end

# ╔═╡ 25139210-aefc-4c60-9853-7b840e2ef373
Markdown.parse("""
## Interpretation

The protection level is the **maximum undetectable position error** along each axis, given:
- The current measurement geometry (runway corner projections)
- The noise model ($(round(px_std, digits=2)) px std dev)
- The false alarm probability ($(prob*100)%)

A smaller PL means the system is more sensitive to faults along that axis. The along-track PL is typically largest because depth estimation from a monocular camera is inherently less precise.

Both solvers should agree closely (within ~3%). The Ipopt solver is ~30x faster due to exact gradients via automatic differentiation + implicit function theorem.
""")
...
RunwayLibProtectionLevels = "e8295940-cb7e-45e5-ae62-4728616c04bd"
...
Rotations = "~1.7.1"
RunwayLib = "~0.1.0"
RunwayLibProtectionLevels = "~0.1.0"
Unitful = "~1.28.0"
"""
...
manifest_format = "2.0"
project_hash = "a855ee919e2cc209a1d6275689391fd6220aba9c"
...
git-tree-sha1 = "6e0ac86a90783f36f6daa496acf39ca138be8922"
uuid = "0a1fb500-61f7-11e9-3c65-f5ef3456f9f0"
version = "0.2.0"
weakdeps = ["ChainRulesCore"]

    [deps.BlockDiagonals.extensions]
...
git-tree-sha1 = "f57029e4aca87d4a18bfae0fe403b126e3c0e9ca"
...
[[deps.OptimizationIpopt]]
deps = ["Ipopt", "LinearAlgebra", "OptimizationBase", "Reexport", "SciMLBase", "SparseArrays", "SymbolicIndexingInterface"]
git-tree-sha1 = "80a57b4424e5ae87635c041e0c26b42802d41be7"
...
[[deps.ProbabilisticParameterEstimators]]
deps = ["Accessors", "BlockDiagonals", "DifferentiationInterface", "Distributions", "DocStringExtensions", "ForwardDiff", "LinearAlgebra", "Logging", "NonlinearSolveBase", "Random", "SciMLBase", "SimpleNonlinearSolve"]
git-tree-sha1 = "1a208e7d17405546cce365f5a7749187463527f6"
uuid = "0098cf3f-3e1e-4f11-b01c-dd9dec1b3374"
version = "0.8.3"

    [deps.ProbabilisticParameterEstimators.extensions]
    TuringExt = "Turing"

    [deps.ProbabilisticParameterEstimators.weakdeps]
...
[[deps.RunwayLib]]
deps = ["ADTypes", "BenchmarkTools", "BracketingNonlinearSolve", "CEnum", "CSV", "DiffEqBase", "DifferentiationInterface", "Distributions", "DocStringExtensions", "ForwardDiff", "LightSumTypes", "LinearAlgebra", "LinearSolve", "Moshi", "NonlinearSolveBase", "NonlinearSolveFirstOrder", "Optimization", "OptimizationOptimJL", "Polyester", "PolyesterWeave", "PrecompileTools", "Printf", "ProbabilisticParameterEstimators", "Rotations", "SciMLBase", "SparseArrays", "StaticArrays", "StaticArraysCore", "StatsBase", "TypedTables", "Unitful"]
path = "/home/romeo/Documents/Stanford/Acubed_collab/RunwayLib_protection-levels"
uuid = "c31d23ff-e9b5-4186-8ef8-ce4d62614c67"
version = "0.1.0"

    [deps.RunwayLib.extensions]
    PythonCallExt = "PythonCall"

    [deps.RunwayLib.weakdeps]
    PythonCall = "6099a3de-0909-46bc-b1f4-468b9a2dfc0d"

[[deps.RunwayLibProtectionLevels]]
deps = ["ADTypes", "BracketingNonlinearSolve", "Chairmarks", "Distributions", "ForwardDiff", "LinearAlgebra", "NonlinearSolveFirstOrder", "Optimization", "OptimizationIpopt", "OptimizationOptimJL", "Printf", "Rotations", "RunwayLib", "SciMLBase", "SparseArrays", "StaticArrays", "Unitful"]
path = "/home/romeo/Documents/Stanford/Acubed_collab/RunwayLib_protection-levels/libs/RunwayLibProtectionLevels"
