...
df = let
    ...
    filter(row -> !ismissing(row.pred_kp_bottom_left_x_px), df_flight)
end
...
function make_noise_model_from_covariance(row)
    ...
    min_eig = minimum(eigvals(cov_matrix))
    if min_eig <= 0
        cov_matrix += (abs(min_eig) + 1e-6) * I
    end
...
function process_trajectory_threaded(df_traj; use_covariance=true, global_sigma=5.0, alpha=0.05)
    ...
    results = tmap(rows) do row
        try
            ...
            pl = compute_protection_levels(cam_pos, cam_rot, pf; alpha=alpha)
